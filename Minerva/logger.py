...
# =====================================================================================================================
class STGLogger():
    def __init__(self, n_batches: int, batch_size: int, out_shape, n_classes: int, record_int: bool = True, 
                 record_float: bool = False) -> None:
        self.record_int = record_int
        self.record_float = record_float
        
        self.logs = {'batch_num' : 0,
                     'total_loss' : 0.0,
                     'total_correct' : 0.0,
                     'labels': None,
                     'predictions': None,
    ...
    def log(self, loss, z, y, bbox, mode: str, step_num, writer) -> None:
        if self.record_int:
        ...
        writer.add_scalar(tag=f'{mode}_loss', scalar_value=ls, global_step=step_num[mode])
        writer.add_scalar(tag=f'{mode}_acc', scalar_value=correct / len(torch.flatten(y)),
                          global_step=step_num[mode])
